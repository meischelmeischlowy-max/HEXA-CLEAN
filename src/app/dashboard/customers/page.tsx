...
export default function DashboardCustomersPage() {
  ...
  const loadCustomers = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("Die Kunden-API hat einen Fehler zurueckgegeben.");
      }
    ...
    <main className="min-h-screen px-4 py-6 sm:px-6 lg:px-8">
      <section className="mx-auto flex w-full max-w-none flex-col gap-5">
        <PageHeader
          ...
          title="Kunden"
          description="Kundenbasis. Status anklicken, dann oeffnet sich der konkrete Kunde mit der passenden Aktion. Keine Rechnungs-, Offerten- oder Auftragsaktionen auf der Liste."
        >
        ...
        {!loading && !errorMessage && customers.length === 0 ? (
          <section className="rounded-3xl border border-white/10 bg-white/[0.03] p-5">
            <EmptyState
              ...
              description="Erstellen Sie den ersten Kunden manuell oder spaeter ueber Formular, Auftrag oder AI Concierge."
            ...
            <div className="flex flex-col gap-2 border-b border-white/10 px-5 py-4 md:flex-row md:items-center md:justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-zinc-500">
                  Status ist die Aktion. Oeffnen/Bearbeiten bleibt nur fuer den
                  Kundendatensatz.
                </p>
              </div>
